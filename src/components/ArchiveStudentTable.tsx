...
export const ArchiveStudentTable: FC<Required<ArchiveStudentTableProps>> = (props) => {
    const data = UserData();
    ...
    return (
        <>
            ...
            <td className="px-6 py-4 text-slate-600">{props.course}</td>
            <td className="px-6 py-4 text-slate-600">{props.section}</td>
            <td className="px-6 py-4 text-slate-600">{props.year}</td>
            <td className="px-6 py-4">
                <span className={`px-2.5 py-1 rounded-full text-xs font-semibold ${
                    props.userRole === "student" ? "bg-emerald-50 text-emerald-700 border border-emerald-100" : "bg-slate-100 text-slate-700"
                }`}>
                    {props.userRole}
                </span>
            </td>
                        ...
                        {isMenuOpen && (
                            <div className="absolute right-0 mt-1 w-48 bg-white rounded-xl shadow-lg border border-slate-200 py-1 z-50 animate-in fade-in zoom-in-95 duration-100">
                                ...
                                >
                                    ...
                                    <span className="font-medium">Restore Student</span>
                                </button>
